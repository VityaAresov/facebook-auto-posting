...
// ---- License Activation/Validation & old migration logic below ----
function activateLicense(licenseKey) {
  console.log("Activating license with Lemon Squeezy:", licenseKey);
  const instanceName = "verification";
  const lemonApiKey =
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..KpEFibw8PM7bKM0bADwPirUNu6hA-KtzaB-XbJ2rin8CNba9HFWrZMWNicOC62eUvPRaPQi-ah9eZaxFL73Fc5jAdPlm0nIN3rPIDcUfQIjDMr2vpGz0i4xg5y0GEfeMBeWHSq0xKVgtjJfmC_-oPJnplCIElPaCRV5cC1r-4GvKJ572gscBXcA3KVuhuj3szYMSBAew4k0MkMqzYNMfOIXIktSjciDJCVuhjnwlkwXJye9V-a-QUSiZmdAqa8yvkBn_HXEvXu2hPhtNWD9vctsdtO_RQWvs5prnNByr6xve-zbDodQuE41Mnv4kvFcIPtAaOA2vIE8ImkB-YS6t_u0FLxkt4vphCs8pFZGPldcxJ0OO3hv4ugdsac9SsfUB8BP7CuMyzK5bDS7vPVa3Rb7VKGXGwgIy_2rzSEIfdcQqX51vJ0Mwn1vfSSZYqUnah8-OOqR_g8yrBlR7pQnbYqtYRa2CPLuDG4c3hNrsU2aYJghhoZ-72pYK1URmAKMG";
  ...
  return new Promise((resolve, reject) => {
    fetch("https://api.lemonsqueezy.com/v1/licenses/activate", {
      ...
      .then(async (activationData) => {
        if (
          activationData &&
          activationData.activated &&
          activationData.license_key.status === "active"
        ) {
          // Fix 2: Use async/await to ensure storage is updated before resolving
          await chrome.storage.local.set({
          ...
          // Fix 3: Add validation result to storage
          await chrome.storage.local.set({
          ...
          console.warn(
            "Lemon Squeezy activation failed. Falling back to Gumroad.",
          );
          fallbackToGumroad(licenseKey, resolve);
        }
      })
      .catch((error) => {
...
function fallbackToGumroad(licenseKey, resolve) {
  const gumroadProductId = "DII2ruZ-rBPeY8fRB4EtLA==";
  fetch("https://api.gumroad.com/v2/licenses/verify", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: `product_id=${gumroadProductId}&license_key=${licenseKey}`,
  })
    .then((response) => response.json())
    .then((gumroadData) => {
      ...
      if (gumroadData && gumroadData.success) {
        const isAllowedEmail =
          gumroadData.purchase.email === "bodoandras4@gmail.com";

        const createdAt = new Date(gumroadData.purchase.created_at);
        const now = new Date();
        ...
        const isWithinOneMonth = monthsSinceCreation <= 1;
        if (isAllowedEmail || isWithinOneMonth) {
          chrome.storage.local.set(
            {
              licenseVerified: true,
              license_key: licenseKey,
              lastValidated: Date.now(),
              licenseProvider: "gumroad",
            },
            () => {
              console.log("Gumroad license saved locally.");
              resolve({ isValid: true });
            },
          );
        } else {
          // License is expired (created more than one month ago)
          console.warn("License expired: created more than one month ago");
          resolve({ isValid: false, message: "Your license has expired." });
        }
      } else {
        console.warn("License is invalid on both Lemon Squeezy and Gumroad.");
...
function validateLicenseKey(licenseKey, licenseProvider) {
  switch (licenseProvider) {
    case "lemonSqueezy":
      console.log("validating lemonSqueezy");
      return validateLemonSqueezyLicense(licenseKey);
    case "gumroad":
      return validateGumroadLicense(licenseKey);
    default:
      console.warn(`Unknown license provider: ${licenseProvider}`);
      return Promise.resolve({
        isValid: false,
        message: "Unknown license provider.",
      });
  }
}

function validateLemonSqueezyLicense(licenseKey) {
  const lemonApiKey =
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..KpEFibw8PM7bKM0bADwPirUNu6hA-KtzaB-XbJ2rin8CNba9HFWrZMWNicOC62eUvPRaPQi-ah9eZaxFL73Fc5jAdPlm0nIN3rPIDcUfQIjDMr2vpGz0i4xg5y0GEfeMBeWHSq0xKVgtjJfmC_-oPJnplCIElPaCRV5cC1r-4GvKJ572gscBXcA3KVuhuj3szYMSBAew4k0MkMqzYNMfOIXIktSjciDJCVuhjnwlkwXJye9V-a-QUSiZmdAqa8yvkBn_HXEvXu2hPhtNWD9vctsdtO_RQWvs5prnNByr6xve-zbDodQuE41Mnv4kvFcIPtAaOA2vIE8ImkB-YS6t_u0FLxkt4vphCs8pFZGPldcxJ0OO3hv4ugdsac9SsfUB8BP7CuMyzK5bDS7vPVa3Rb7VKGXGwgIy_2rzSEIfdcQqX51vJ0Mwn1vfSSZYqUnah8-OOqR_g8yrBlR7pQnbYqtYRa2CPLuDG4c3hNrsU2aYJghhoZ-72pYK1URmAKMG";
  return fetch("https://api.lemonsqueezy.com/v1/licenses/validate", {
    ...
    .then(async (validationData) => {
      if (validationData?.valid) {
        console.log("Lemon Squeezy license validated.");
        await chrome.storage.local.set({
          lastValidated: Date.now(),
          licenseVerified: true,
        });
        ...
        console.warn("Lemon Squeezy license validation failed.");
...
function validateGumroadLicense(licenseKey) {
  const gumroadProductId = "DII2ruZ-rBPeY8fRB4EtLA==";
  return fetch("https://api.gumroad.com/v2/licenses/verify", {
    ...
    .then((response) => response.json())
    .then(async (gumroadData) => {
      if (gumroadData && gumroadData.success) {
        console.log("Gumroad license validated.");
        const isAllowedEmail =
          gumroadData.purchase.email === "bodoandras4@gmail.com";
        ...
        // Allow if it's the approved email or if it's within one month of creation
        if (isAllowedEmail || isWithinOneMonth) {
          await chrome.storage.local.set({
            lastValidated: Date.now(),
            licenseVerified: true,
        ...
        console.warn("Gumroad license validation failed.");
...
async function preprocessMedia(media, compressImages) {
  // Input Validation
  if (!media || !media.type || !media.data) {
    return { type: "invalid", data: null };
  }

  // If already a stored reference (scheduled post), return as is
  if (media.type === "stored_video") {
    return media;
  }

  console.log(`[MediaEngine] Processing ${media.type}...`);

  try {
    // --- IMAGE PIPELINE ---
    if (media.type === "image") {
      const imageBlob = dataURItoBlob(media.data);
      const compressedBlob = await compressImage(imageBlob);
      return {
        type: "image",
        data: await blobToDataURI(compressedBlob),
      };
    }

    // --- VIDEO PIPELINE (NEW: IndexedDB Storage) ---
    else if (media.type === "video") {
      const videoBlob = dataURItoBlob(media.data);

      // 1. Hash Buster (Magic Byte)
      const randomId = Math.random().toString(36).substring(7);
      const noiseBlob = new Blob([`_gpp_unique_${randomId}`], {
        type: "text/plain",
      });
      const uniqueVideoBlob = new Blob([videoBlob, noiseBlob], {
        type: videoBlob.type,
      });

      console.log(
        `[MediaEngine] Video processed. Size: ${(
          uniqueVideoBlob.size /
          1024 /
          1024
        ).toFixed(2)}MB`,
      );
      ...
      // 2. Save to IndexedDB
      const videoId = `vid_${Date.now()}_${randomId}`;
      await saveVideoToDB(videoId, uniqueVideoBlob, videoBlob.type);

      console.log(`[MediaEngine] Video stored in IDB with ID: ${videoId}`);

      // 3. Return Reference instead of Data
      return {
        type: "stored_video",
        id: videoId,
...
async function compressImage(blob) {
  if (!(blob instanceof Blob)) {
    throw new Error("compressImage expects a Blob object");
  }
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        // 1. Smart Resizing (Cap at 1920px for 1080p HD)
        // Larger than this wastes CPU/RAM as FB resizes anyway.
        ...
        // Use OffscreenCanvas for background performance
        ...
        // 3. Anti-Ban Pixel Jitter (The "Unique-ifier")
        // We modify the bottom-right pixel slightly
        const cornerData = context.getImageData(width - 1, height - 1, 1, 1);
        const d = cornerData.data;
        // Randomly shift Red channel by +/- 1
        d[0] = d[0] > 128 ? d[0] - 1 : d[0] + 1;
        context.putImageData(cornerData, width - 1, height - 1);

        // 4. Quality Jitter (Randomize compression level)
        // Range: 0.75 to 0.85
        const randomQuality = 0.75 + Math.random() * 0.1;
        ...
        console.error("Error in optimized compressor:", error);
        reject(error);
      } finally {
...
async function blobToDataURI(blob) {
  if (!(blob instanceof Blob)) {
    throw new Error("blobToDataURI expects a Blob object");
  }
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
...
// --- ADD this new helper function to check license status ---
async function checkLicenseStatus() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      ["licenseVerified", "freePostsRemaining"],
      (result) => {
        const hasLicense = !!result.licenseVerified;
        const hasFreeTrial = result.freePostsRemaining > 0;
        ...
        if (hasLicense) {
          ...
          resolve({
            isAuthorized: true,
            message: "Using free trial",
          });
        } else {
          resolve({
            isAuthorized: false,
            message:
              "AI features require a valid license or free trial. Please upgrade or activate your license to use AI features.",
          });
        }
      },
    );
...
// --- ADDED: Configuration for API Key Retrieval ---
const API_KEY_ENDPOINT =
  "https://groupposting.com/wp-json/groupposting/v1/get-api-key";
const VALIDATION_KEY = "goodFAITH";
const EXTENSION_ID = "groupposting-extension";

// --- UPDATED: Function to securely get the API key with Persistent Caching ---
async function getSecureApiKey() {
  try {
    // 1. Try to get cached key from storage first
    const { aiApiKeyData } = await chrome.storage.local.get("aiApiKeyData");
    const now = Date.now() / 1000; // Seconds

    // 2. Check if valid
    if (aiApiKeyData && aiApiKeyData.key && aiApiKeyData.expires > now + 60) {
      console.log("Using cached API key from storage.");
      return aiApiKeyData.key;
    }

    // 3. Fetch new key if missing or expired
    ...
    console.log("New API key cached successfully.");
    return data.api_key;
  } catch (error) {
...
async function executeSingleDirectApiPost(
  ...
  const groupLinkURL = groupLink?.[1];
  const groupLinkTitle = groupLink?.[0] || "Untitled Group";
  let logEntry = {
    linkTitle: groupLinkTitle,
    linkURL: groupLinkURL || "N/A",
  ...
  try {
    await sleep(getRandomDelay(1, 12));
    const state = await getPostingState();

    // Check the persisted stopRequested flag
    if (state.stopRequested) {
      throw new Error("Stop requested");
    }
    ...
    // This part should not be reached if postTo... throws, but as a fallback:
    logEntry.response = result.success ? "successful" : "failed";
    logEntry.reason = result.success
      ? null
      : result.message || "API post failed";
    logEntry.method = `direct_api${
      result.success ? "" : "_" + (result.errorType || "failed")
    }`;
  } catch (error) {
    // *** THIS IS THE FIX for [object Object] ***
    logEntry.response = "failed";
    // Check if error is an object with a message, otherwise stringify it.
    logEntry.reason = error.message ? error.message : JSON.stringify(error);
    logEntry.method = `direct_api_${error.errorType || "error"}`;
    // The main process will handle the fallback logic if this throws an error.
    // We DON'T re-throw here because we want the resilient loop to continue.
  }
  return { log: logEntry };
}
